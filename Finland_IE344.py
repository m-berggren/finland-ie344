...
from setup import get_desktop_path, get_template_file, open_filedialog, show_messagebox

def create_manifest():
    """
    Script is used to merge data from two chosen excel files, first file (ELL)
    ...
    # Choose files to load data from. First file is ELL, second MRN-file.
    ...
    df_mrn = df_mrn.rename(columns={'Container No': 'Marks & Nos'}) # Renames column to another.
    df_mrn = df_mrn.loc[: ,['Marks & Nos', 'MRN', 'Sequence No']]   # df_mrn now only consists of these 3 columns.
    df_merged = df_ell.merge(df_mrn, how='left', on='Marks & Nos')  # merges the 3 columns in df_mrn into df_ell.
